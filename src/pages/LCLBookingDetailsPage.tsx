...
import { FaUser, FaBuilding, FaEnvelope, FaPhone, FaArrowLeft, FaInfoCircle, FaBoxes, FaRupeeSign, FaClipboardList, FaShieldAlt, FaFileAlt, FaArrowRight } from 'react-icons/fa';
...
const LCLBookingDetailsPage: React.FC = () => {
  ...
  const insuranceAmount = insuranceRequired && selectedResult ? selectedResult.price * 0.01 : 0;
  const totalAmount = selectedResult ? selectedResult.price + insuranceAmount : 0;
  ...
  const handleFinalConfirmBooking = async () => {
    ...
    const bookingId = `LCL-${Date.now().toString().slice(-6)}-${Math.floor(Math.random() * 1000)}`;

    try {
      await bookingAPI.create({
        booking_number: bookingId,
        service_type: 'LCL',
        origin: selectedResult.originWarehouse,
        destination: selectedResult.destinationWarehouse,
    ...
    const bookingDetails = {
      selectedResult: {
        ...
        originStation: selectedResult.originWarehouse,
        destinationStation: selectedResult.destinationWarehouse,
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-4 sm:p-6 flex flex-col items-center justify-center">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center">
          ...
          <button onClick={() => navigate('/lcl-results')} className="bg-brand-gradient hover:opacity-90 text-white font-bold py-3 px-8 rounded-full shadow-md transition duration-300">
          ...
          <div className="w-full md:flex-grow">
            ...
            <div className="bg-blue-50 p-6 rounded-xl border border-blue-200 shadow-md mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2 text-gray-700">
                <div><strong>Service:</strong> {selectedResult.serviceName}</div>
                <div><strong>Carrier:</strong> {selectedResult.carrier}</div>
                <div><strong>Origin:</strong> {selectedResult.originWarehouse}</div>
                <div><strong>Destination:</strong> {selectedResult.destinationWarehouse}</div>
                <div><strong>Ready Date:</strong> {selectedResult.readyDate}</div>
                <div><strong>Transit:</strong> {selectedResult.transitTime}</div>
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 mb-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea rows={3} className="focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md p-2" placeholder="Any specific delivery instructions." value={specialInstructions} onChange={e => setSpecialInstructions(e.target.value)} />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 mb-8">
              <label className="flex items-center cursor-pointer">
                <input type="checkbox" checked={insuranceRequired} onChange={e => setInsuranceRequired(e.target.checked)} className="form-checkbox h-5 w-5 text-blue-600 rounded mr-3" />
                <div>
                  <span className="text-lg font-semibold text-gray-800">Add Cargo Insurance</span>
                  <p className="text-sm text-gray-500">Protect your cargo against loss or damage during transit.</p>
                </div>
              </label>
            </div>
            ...
            <div className="sticky top-4 bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
              ...
              <div className="text-gray-700">
                <div className="flex justify-between py-1">
                  ...
                  <span className="font-semibold">₹{selectedResult.price.toLocaleString('en-IN')}</span>
                </div>
                {insuranceRequired && (
                  <div className="flex justify-between py-1">
                    <span>Insurance</span>
                    <span className="font-semibold">₹{insuranceAmount.toLocaleString('en-IN')}</span>
                  </div>
                )}
              ...
              <p className="text-sm text-gray-500 mt-4">*Prices are indicative and subject to change.</p>
            </div>
          </div>
        </div>
      </div>
