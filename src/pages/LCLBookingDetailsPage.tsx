// src/pages/LCLBookingDetailsPage.tsx
import React, { useState } from 'react';
import { useLocation, useNavigate } from 'react-router-dom';
import { FaUser, FaBuilding, FaEnvelope, FaPhone, FaArrowLeft, FaInfoCircle, FaBoxes, FaRupeeSign, FaClipboardList, FaShieldAlt, FaFileAlt } from 'react-icons/fa';
...
// Define the structure for an LCL service result, matching what's passed from LCLResultsPage
...
interface LCLBookingDetailsPageProps {
  // Props will be passed via location.state
}

const LCLBookingDetailsPage: React.FC<LCLBookingDetailsPageProps> = () => {
  const location = useLocation();
  const navigate = useNavigate();

  // Retrieve data passed from LCLResultsPage
  const selectedResult = location.state?.selectedResult as LCLServiceResult | undefined;
  const originalFormData = location.state?.originalFormData as LCLFormData | undefined;
  ...
  const handleFinalConfirmBooking = () => {
    ...
    const bookingDetails = {
      selectedResult: { // Map LCL result to a generic structure for confirmation page
        id: selectedResult.id,
        serviceName: selectedResult.serviceName,
        originStation: selectedResult.originWarehouse, // Use originWarehouse from LCLServiceResult
        destinationStation: selectedResult.destinationWarehouse, // Use destinationWarehouse from LCLServiceResult
        departureTime: 'N/A', // LCL might not have fixed departure times
        arrivalTime: 'N/A', // LCL might not have fixed arrival times
        transitTime: selectedResult.transitTime,
        price: selectedResult.price,
        availableCapacity: originalFormData.weight ? `${originalFormData.weight} KG` : 'N/A', // Using weight as capacity indicator
        features: selectedResult.features,
        operator: selectedResult.carrier, // Use carrier from LCLServiceResult
      ...
      bookingId: `LCL-${Date.now().toString().slice(-6)}-${Math.floor(Math.random() * 1000)}`,
    };
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-4 sm:p-6 flex flex-col items-center justify-center">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center">
          ...
          <button
            onClick={() => navigate('/lcl-results')} // Navigate back to LCL results
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 flex flex-col items-center">
      <div className="w-full max-w-7xl bg-white rounded-2xl shadow-xl">
        ...
        <div className="bg-blue-600 text-white p-6 rounded-t-2xl flex items-center justify-between">
          <h1 className="text-3xl font-bold">Complete Your LCL Booking</h1>
          <button
        ...
        <div className="flex flex-col md:flex-row gap-6 p-4 sm:p-6">
          ...
          <div className="w-full md:flex-grow">
            {/* Selected LCL Service Summary */}
            <div className="bg-blue-50 p-6 rounded-xl border border-blue-200 shadow-md mb-6">
              <h2 className="text-xl font-bold text-blue-800 mb-4 flex items-center">
                <FaBoxes className="text-teal-600 mr-3 text-2xl" /> Selected LCL Service
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2 text-gray-700">
                ...
                <div><strong>Carrier:</strong> {selectedResult.carrier}</div>
                <div><strong>Origin Warehouse:</strong> {selectedResult.originWarehouse}</div>
                <div><strong>Destination Warehouse:</strong> {selectedResult.destinationWarehouse}</div>
                <div><strong>Ready Date:</strong> {selectedResult.readyDate}</div>
                <div><strong>Transit Duration:</strong> {selectedResult.transitTime}</div>
                <div><strong>Dimensions:</strong> {selectedResult.dimensions}</div>
            ...
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-md mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-2 text-gray-700">
                <DetailRow label="Origin" value={originalFormData.origin} />
                <DetailRow label="Destination" value={originalFormData.destination} />
                <DetailRow label="Ready Date" value={originalFormData.date} />
                ...
                <DetailRow label="Dimensions" value={originalFormData.dimensions || 'N/A'} />
                <DetailRow label="Volumetric Weight" value={originalFormData.volumetricWeight ? `${originalFormData.volumetricWeight} KG` : 'N/A'} />
                <DetailRow label="Hazardous Cargo" value={originalFormData.hazardousCargo ? 'Yes' : (originalFormData.hazardousCargo === false ? 'No' : 'N/A')} />
                <DetailRow label="Incoterms" value={originalFormData.incoterms || 'N/A'} />
                <DetailRow label="Insurance Required (Initial)" value={originalFormData.insuranceRequired ? 'Yes' : (originalFormData.insuranceRequired === false ? 'No' : 'N/A')} />
                <div className="flex items-start lg:col-span-3">
                  <FaInfoCircle className="mr-2 text-blue-500 mt-1 flex-shrink-0" />
                  <div className="flex justify-between w-full">
                    <span className="font-semibold text-gray-700 text-sm flex-shrink-0">Cargo Value:</span>
                    <span className="text-gray-800 font-medium text-base whitespace-nowrap ml-2 flex-grow">INR {originalFormData.value ? originalFormData.value.toLocaleString('en-IN') : 'N/A'}</span>
                  </div>
                </div>
...
export default LCLBookingDetailsPage;
