...
import { FaUser, FaBuilding, FaEnvelope, FaPhone, FaArrowLeft, FaInfoCircle, FaStamp, FaRupeeSign, FaClipboardList, FaShieldAlt, FaFileAlt, FaArrowRight } from 'react-icons/fa';
...
const CustomsBookingDetailsPage: React.FC = () => {
  ...
  const handleFinalConfirmBooking = async () => {
    ...
    const bookingId = `CUST-${Date.now().toString().slice(-6)}-${Math.floor(Math.random() * 1000)}`;
    ...
    const bookingDetails = {
      selectedResult: {
        ...
        originStation: originalFormData.origin || 'N/A',
        destinationStation: originalFormData.destination || 'N/A',
        transitTime: selectedResult.estimatedTime,
        price: totalAmount,
        features: selectedResult.features,
        operator: selectedResult.provider,
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-4 sm:p-6 flex flex-col items-center justify-center">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center">
          ...
          <button onClick={() => navigate('/customs-results')} className="bg-brand-gradient hover:opacity-90 text-white font-bold py-3 px-8 rounded-full shadow-md transition duration-300">
