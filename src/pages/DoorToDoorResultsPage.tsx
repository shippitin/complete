...
import { FaHome, FaInfoCircle, FaArrowLeft, FaTag, FaClock, FaRupeeSign, FaTruckMoving, FaKey } // FaKey for features
  from 'react-icons/fa';
import type { DoorToDoorFormData } from '../types/QuoteFormHandle';

// Define the structure for a Door to Door service result
...
const DoorToDoorResultsPage: React.FC = () => {
  ...
  // Function to go back to the QuoteFormPage (HeroSection)
  const handleBackToForm = () => {
    navigate('/', { state: { activeTab: 'Door to Door' } });
  };
  ...
  const generateDummyResults = (data: DoorToDoorFormData): DoorToDoorServiceResult[] => {
    ...
    return [
      {
        id: 'D2D-001',
        serviceName: 'Express Doorstep Delivery',
        carrier: 'FastFreight Logistics',
        transitTime: '2-3 Days',
        price: Math.round(basePrice * weightFactor * distanceFactor * 1.2),
        features: ['Real-time Tracking', 'Dedicated Vehicle', 'Priority Handling'],
        status: 'Available',
        isSpecialOffer: true,
      ...
      {
        id: 'D2D-002',
        serviceName: 'Standard Door to Door',
        carrier: 'Reliable Carriers Pvt. Ltd.',
        transitTime: '4-6 Days',
        price: Math.round(basePrice * weightFactor * distanceFactor),
        features: ['Economical', 'Standard Tracking'],
        status: 'Available',
      },
      {
        id: 'D2D-003',
        serviceName: 'Economy Ground Service',
        carrier: 'Budget Freight Solutions',
        transitTime: '7-10 Days',
        price: Math.round(basePrice * weightFactor * distanceFactor * 0.8),
        features: ['Cost-Effective', 'Bulk Shipments'],
        status: 'Limited',
      },
      {
        id: 'D2D-004',
        serviceName: 'Premium Express (Full)',
        carrier: 'Top Tier Logistics',
        transitTime: '1-2 Days',
        price: Math.round(basePrice * weightFactor * distanceFactor * 1.5),
        features: ['Urgent Delivery', 'White Glove Service'],
        status: 'Full',
      },
    ];
  };
      ...
      <div className="min-h-screen bg-gray-50 p-4 sm:p-6 flex flex-col items-center justify-center font-inter">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center border border-gray-100">
          ...
          <button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-full shadow-md transition duration-300 transform hover:scale-105"
          >
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 font-inter">
      <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl p-6 md:p-8 border border-gray-100">
        ...
        {/* Special Offer Banner - Replicated from Screenshot 2, with subtler blue */}
        ...
        {dummyResults.length > 0 ? (
          <div className="grid grid-cols-1 gap-6">
            {dummyResults.map((result) => (
              <div key={result.id} className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 flex flex-col justify-between">
                <div>
                  ...
                  <div className="space-y-2 mb-4 text-gray-700">
                    <div className="flex items-center">
                      ...
                      <span className="text-sm">Estimated Transit Time: <span className="font-semibold">{result.transitTime}</span></span>
                    </div>
                    <div className="flex items-center">
                      <FaTruckMoving className="text-gray-500 mr-2 text-sm" />
                      <span className="text-sm">Service Type: Door to Door</span>
                    </div>
                ...
                {/* Price and Book Now - Replicated from Screenshot 2, with price in black/dark gray */}
                <div className="mt-auto pt-4 border-t border-gray-100 flex justify-between items-center">
                  <div className="flex items-center">
                    <FaRupeeSign className="text-xl text-gray-700 mr-1" /> {/* Changed icon color to text-gray-700 */}
                    <span className="text-2xl font-bold text-gray-900"> {/* Changed price text color to text-gray-900 */}
                      {result.price === 0 ? 'Price on Request' : result.price.toLocaleString('en-IN')}
                    </span>
                  </div>
                  <button
                    onClick={() => handleBookNow(result)}
                    className={`py-2 px-6 rounded-md text-white font-semibold text-base shadow-sm transition duration-300 ease-in-out
                                ${result.status === 'Full' ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-400 hover:bg-blue-500'}`}
          ...
          <div className="text-center py-10 text-gray-500 bg-gray-50 rounded-lg shadow-inner border border-gray-100">
            ...
            <button
              onClick={handleBackToForm}
              className="mt-6 bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-6 rounded-full shadow transition duration-300"
            >
