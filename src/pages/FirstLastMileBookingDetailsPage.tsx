...
import { FaUser, FaBuilding, FaEnvelope, FaPhone, FaArrowLeft, FaInfoCircle, FaRupeeSign, FaClipboardList, FaShieldAlt, FaFileAlt, FaArrowRight, FaTruck } from 'react-icons/fa';
import { bookingAPI } from '../services/api';

const FirstLastMileBookingDetailsPage: React.FC = () => {
  ...
  const subtotal = selectedResult?.price || 0;
  const taxesAndFees = subtotal * 0.18;
  const insuranceCost = insuranceRequired ? subtotal * 0.03 : 0;
  const expressHandlingCost = expressHandling ? 150 : 0;
  ...
  const handleFinalConfirmBooking = async () => {
    ...
    const bookingId = `FLM-${Date.now().toString().slice(-6)}-${Math.floor(Math.random() * 1000)}`;

    try {
      await bookingAPI.create({
        booking_number: bookingId,
        service_type: 'First/Last Mile',
        origin: originalFormData.origin || 'N/A',
        destination: originalFormData.destination || 'N/A',
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-4 sm:p-6 flex flex-col items-center justify-center">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center">
          ...
          <button onClick={() => navigate('/first-last-mile-results')} className="bg-brand-gradient hover:opacity-90 text-white font-bold py-3 px-8 rounded-full shadow-md transition duration-300">
          ...
          <div className="w-full md:flex-grow">
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 mb-8">
              <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center">
                <FaShieldAlt className="mr-2 text-blue-500" /> Additional Services
              </h3>
              <div className="space-y-3">
                <label className="flex items-center cursor-pointer">
                  ...
                  <div>
                    <span className="font-semibold text-gray-800">Add Cargo Insurance</span>
                    <p className="text-sm text-gray-500">Protect your cargo against loss or damage.</p>
                  </div>
                </label>
                <label className="flex items-center cursor-pointer">
                  <input type="checkbox" checked={expressHandling} onChange={e => setExpressHandling(e.target.checked)} className="form-checkbox h-5 w-5 text-blue-600 rounded mr-3" />
                  <div>
                    <span className="font-semibold text-gray-800">Express Handling (+₹150)</span>
                    <p className="text-sm text-gray-500">Expedite your shipment for faster processing.</p>
                  </div>
                </label>
              </div>
            </div>
          ...
          <div className="w-full md:w-72 lg:w-80 flex-shrink-0">
            <div className="sticky top-4 bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
              ...
              <div className="text-gray-700 space-y-2">
                <div className="flex justify-between"><span>Base Price</span><span className="font-semibold">₹{subtotal.toLocaleString('en-IN')}</span></div>
                <div className="flex justify-between"><span>Taxes (18%)</span><span className="font-semibold">₹{taxesAndFees.toLocaleString('en-IN')}</span></div>
                {insuranceRequired && <div className="flex justify-between"><span>Insurance</span><span className="font-semibold">₹{insuranceCost.toLocaleString('en-IN')}</span></div>}
                {expressHandling && <div className="flex justify-between"><span>Express Handling</span><span className="font-semibold">₹{expressHandlingCost.toLocaleString('en-IN')}</span></div>}
