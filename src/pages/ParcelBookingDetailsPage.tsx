...
import { FaUser, FaBuilding, FaEnvelope, FaPhone, FaArrowLeft, FaInfoCircle, FaBox, FaRupeeSign, FaClipboardList, FaShieldAlt, FaFileAlt, FaArrowRight, FaCheckCircle } from 'react-icons/fa';
...
const ParcelBookingDetailsPage: React.FC = () => {
  ...
  const handleFinalConfirmBooking = async () => {
    ...
    const bookingId = `PARCEL-${Date.now().toString().slice(-6)}-${Math.floor(Math.random() * 1000)}`;

    try {
      await bookingAPI.create({
        booking_number: bookingId,
        service_type: 'Parcel',
        origin: selectedResult.origin || originalFormData.origin || 'N/A',
        destination: selectedResult.destination || originalFormData.destination || 'N/A',
        cargo_type: selectedResult.cargoType || 'General',
        weight: selectedResult.totalWeight || originalFormData.totalWeight || 0,
          ...
          <div className="w-full md:flex-grow">
            ...
            <div className="bg-blue-50 p-6 rounded-xl border border-blue-200 shadow-md mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2 text-gray-700">
                <div><strong>Provider:</strong> {selectedResult.serviceProvider}</div>
                ...
                <div><strong>Type:</strong> {selectedResult.isDomestic ? 'Domestic' : 'International'}</div>
                <div><strong>Weight:</strong> {selectedResult.totalWeight} Kgs</div>
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 mb-8">
                ...
                <div>
                  ...
                  <div className="relative rounded-md shadow-sm">
                    ...
                    <select className="focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 sm:text-sm border-gray-300 rounded-md p-2" value={kycDocType} onChange={e => setKycDocType(e.target.value)}>
                      {kycDocOptions.map(option => <option key={option} value={option === 'Select Document Type' ? '' : option}>{option}</option>)}
                    </select>
                  </div>
                </div>
              ...
              <div className="mt-6 border-t pt-4 border-gray-200">
                <label className="flex items-center cursor-pointer">
                  <input type="checkbox" checked={insuranceRequired} onChange={e => setInsuranceRequired(e.target.checked)} className="h-5 w-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                  <span className="ml-3 text-base font-medium text-gray-800">Add Cargo Insurance (Optional)</span>
                </label>
              </div>
            </div>
            ...
            <div className="flex justify-end mt-8">
              <button
                ...
                disabled={loading}
                className="flex items-center px-8 py-4 bg-green-600 text-white font-bold text-xl rounded-xl shadow-lg hover:bg-green-700 transition duration-300 disabled:opacity-50"
              >
                {loading ? <span className="animate-spin rounded-full h-5 w-5 border-2 border-white border-t-transparent mr-2"></span> : null}
                {loading ? 'Saving...' : <><span>Confirm Booking</span><FaArrowRight className="ml-3" /></>}
              </button>
            </div>
          </div>
